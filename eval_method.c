...
search_method(VALUE klass, ID id, VALUE *klassp)
{
    ...
    if (!klass) {
#if WITH_OBJC
	/* TODO: if saved_klass's objc class responds to id, we should define
	   the objc closure 
	Class ocklass = RCLASS(saved_klass)->ocklass;
	body = rb_objc_define_objc_mid_closure( */
	return 0;
#else
	return 0;
#endif
