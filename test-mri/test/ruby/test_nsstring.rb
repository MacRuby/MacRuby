...
class TestNSString < Test::Unit::TestCase

  def initialize(*args)
    @cls = NSStringWraper.new
    @aref_re_nth = true
...
=end
  ...
  def test_times
    assert_raise(ArgumentError) { S("a") * (-1) }
  end
  ...
  def test_match_method
    ...
    assert_equal("foobarbaz", S("foo").match(o, S("bar"), "baz"))
    x = nil
  ...
  def test_partition
    ...
    def (hyphen = Object.new).to_str; "-".to_ns; end
