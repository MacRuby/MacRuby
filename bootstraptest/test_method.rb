...
assert_equal 'ok', %q{
  ...
  o = NSObject.new
  k = NSString.new
  d.performSelector('setObject:forKey:', withObject:o, withObject:k)
  d.objectForKey(k) == o ? :ok : :nok
}
...
assert_equal 'ok', %q{
  ...
  Foo.new.foo(1, x:2) == 2 ? :ok : :nok
}

assert_equal 'ok', %q{
  class Foo
    def foo(a, x:b, x:c); b+c; end 
  end
  Foo.new.foo(1, x:2, x:3) == 5 ? :ok : :nok
}

=begin
# TODO: Make these tests working with the current framework.
#       (No exception is raised by the parser, so we need to figure another way.)
assert_equal 'ok', %q{
  begin
    class Foo
      def foo(x:a); end
