...
RoxorVM::RoxorVM(const RoxorVM &vm)
{
    ...
    replace_outer_stack(vm.outer_stack);
	...
	rb_vm_outer_t *outer)
{
    ...
    if (outer == NULL) {
	// KOUJI_TODO: fix to get outer method
	rb_vm_get_outer(&outer);
	got_outer = true;
    }
    struct Finally {
	bool got_outer;
	rb_vm_outer_t *outer;
	Finally(bool _got_outer, rb_vm_outer_t *_outer) {
	    got_outer = _got_outer;
	    outer = _outer;
	}
	~Finally() {
	    if (got_outer) {
		rb_vm_release_outer(&outer);
	    }
	}
    } finalizer(got_outer, outer);
	...
	GC_REF(&real_node->outer, NULL);
    }
    else {
	GC_REF(&real_node->outer, outer);
    }
    ...
    GC_REPLACE(&node->outer, real_node->outer);
    if (imp != ruby_imp) {
	...
	if (included_in_classes != Qnil) {
...
#if ROXOR_VM_DEBUG
		printf("forward %c[%s %s] with imp %p node %p types %s outer %p\n",
		...
		node->sel = sel;
		GC_UNREF(&node->outer);
		GC_REF(&node->outer, real_node->outer);
	    }
	}
    }
	...
	bool lexical, bool defined, rb_vm_outer_t *outer_stack)
{
	...
	for (rb_vm_outer_t *o = root_outer; o != NULL && o->outer != NULL; o = o->outer) {
	    if (o->pushed_by_eval) {
		continue;
	    }
...
get_klass_const(VALUE outer, ID path, bool lexical)
{
	...
	struct Finally {
	    rb_vm_outer_t *outer;
	    Finally(rb_vm_outer_t *_outer) {
		outer = _outer;
	    }
	    ~Finally() {
		rb_vm_release_outer(&outer);		
	    }
	} finalizer(outer_stack);
...
vm_alias_method(Class klass, Method method, ID name, bool noargs)
{
	...
	GET_CORE()->add_method(klass, sel, imp, node->ruby_imp,
		node->arity, node->flags, types, node->outer);
    }
    else {
	class_replaceMethod(klass, sel, imp, types);
    }
...
{
    if (dynamic_class) {
	outer = (VALUE)GET_VM()->get_outer_stack()->klass;
...
rb_vm_defined(VALUE self, int type, VALUE what, VALUE what2)
{
	...
	case DEFINED_LCONST:
		...
		if (rb_vm_const_lookup(what2, (ID)what, type == DEFINED_LCONST, true, o)) {
		    str = "constant";
		}
	    }
	    break;
	...
	const char *old_types, const char *new_types)
{
    ...
    return add_method(klass, node->sel, node->objc_imp, node->ruby_imp,
	    node->arity, node->flags, new_types, node->outer);
#endif
}
...
RoxorCore::resolve_method(Class klass, SEL sel, void *func,
	...
	void *objc_imp_types, rb_vm_outer_t *outer)
{
#if MACRUBY_STATIC
    assert(imp != NULL);
#else
    ...
    return add_method(klass, sel, objc_imp, imp, arity, flags, types, outer);
}
...
RoxorCore::prepare_method(Class klass, SEL sel, Function *func,
	const rb_vm_arity_t &arity, int flags, rb_vm_outer_t *outer)
{
#if ROXOR_VM_DEBUG
    printf("preparing %c[%s %s] on class %p LLVM func %p flags %d outer %p\n",
	    class_isMetaClass(klass) ? '+' : '-',
	    class_getName(klass),
	    sel_getName(sel),
	    klass,
...
prepare_method:
	...
	if (m != NULL || custom_resolver) {
	    ...
	    GET_CORE()->prepare_method(klass, sel, func, arity, flags, outer);
	}
	GET_CORE()->unlock();
#endif
    }
...
RoxorCore::copy_methods(Class from_class, Class to_class)
{
	...
	if (m != NULL) {
	    ...
	    resolve_method(to_class, sel, m_src->func, m_src->arity,
		    m_src->flags, imp, m, NULL, m_src->outer);
	}
	else {
#if ROXOR_VM_DEBUG
...
rb_vm_define_method(Class klass, SEL sel, IMP imp, NODE *node, bool direct)
{
    ...
    return __rb_vm_define_method(klass, sel, imp, imp, rb_vm_node_arity(node),
	    rb_vm_node_flags(node), direct, NULL);
}
	...
	long flags, bool direct)
{
    ...
    return __rb_vm_define_method(klass, sel, node->objc_imp, node->ruby_imp,
	    node->arity, flags, direct, node->outer);
}
...
{
    GC_REF(&outer_stack, create_outer(klass, outer_stack, false));
...
RoxorVM::pop_outer(void)
{
    ...
    assert(old != NULL);
    GC_REF(&outer_stack, outer_stack->outer);
    GC_UNREF(&old);

#if ROXOR_VM_DEBUG_OUTER
    rb_vm_print_outer_stack(NULL, NULL, __FUNCTION__, __LINE__,
	    outer_stack, "popped outer");
#endif
}
...
{
    GC_REPLACE(&outer_stack, new_outer_stack);
}

void
rb_vm_set_outer(rb_vm_outer_t *outer)
{
    GET_VM()->replace_outer_stack(outer);
}

void
rb_vm_get_outer(rb_vm_outer_t **dest_ptr)
{
    GC_REF(dest_ptr, GET_VM()->get_outer_stack());
}

void
rb_vm_release_outer(rb_vm_outer_t **dest_ptr)
{
    GC_UNREF(dest_ptr);
}
...
rb_vm_create_block_from_method(rb_vm_method_t *method)
{
    ...
    b->klass = 0;
    if (method->node == NULL) {
	// KOUJI_TODO: implement if method->node == NULL
	b->outer = NULL;
    }
    else {
	GC_REF(&b->outer, method->node->outer);
    }
...
//func->dump();
	...
	mnode->flags = 0;
	rb_vm_release_outer(&mnode->outer);
	rb_vm_get_outer(&mnode->outer);
	// KOUJI_TODO: where and who does mnode delete?
	...
	rb_vm_binding_t *binding, bool inside_eval, bool should_push_outer)
{
    ...
    rb_vm_get_outer(&old_outer_stack);
	...
	~Finally() { 
	    ...
	    rb_vm_set_outer(old_outer_stack);
	    rb_vm_release_outer(&old_outer_stack);
...
Init_VM(void)
{
    ...
    GET_VM()->push_outer((Class)rb_cObject);
